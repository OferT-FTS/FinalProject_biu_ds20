...
def intro():
    ...
    with col1:
        st.markdown(
            "### Presenter\n- Ofer Tzvi\n- Technology, Data Science, Risk Management\n\n### Objectives\n- Present Project's Core Concepts\n- Discuss important project's phases\n- Present Model Results")
    with col2:
        st.markdown(
            "### Duration\n- ~25–30 minutes (adjustable)\n\n### Audience\n- Data practitioners, Engineers & Academic staff")
    st.markdown("---")
...
def page_evaluation():
    ...
    st.image("https://upload.wikimedia.org/wikipedia/commons/1/1b/K-fold_cross_validation_EN.svg",
...
def page_dl_overview():
    ...
    st.markdown(
        "**Deep learning**: large models, representation learning — excels with images, audio, text.\nChallenges: data hunger, interpretability, compute cost.")

def page_keras_example():
    header("Neural Network Example (Keras)", "Train a small MLP on the sklearn digits dataset")
    st.write(
        "We will train a compact neural network on the `digits` dataset (handwritten 8x8 images). This runs quickly for demo purposes.")
    ...
    if st.button('Train Neural Network'):
        ...
        st.success(
            'Training completed — examine tradeoffs and consider regularization, more data, or architecture changes')

def page_cnn_overview():
    header("Computer Vision & Convolutional Neural Networks", "Core ideas & popular architectures")
    st.markdown(
        "**Convolutional layers** detect local patterns; pooling reduces spatial size; modern CV uses CNN backbones like ResNet, EfficientNet.")
...
def main():
    fn = PAGE_DISPATCH.get(selection, intro)
    fn()
    st.sidebar.markdown('---')
